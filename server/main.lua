...
RegisterNetEvent('kashacter:getCharacterData')
AddEventHandler('kashacter:getCharacterData', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)

	-- Obtener solo los datos del personaje (modelo y skin)
	MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier', {
		['@identifier'] = xPlayer.identifier
	}, function(result)
		if result[1] then
			local characterData = {
				skin = json.decode(result[1].skin),           -- Datos del skin del personaje
				model = result[1].model or 'mp_m_freemode_01' -- Modelo del personaje
			}

			-- Enviar solo los datos del personaje al cliente
			TriggerClientEvent('kashacter:sendCharacterData', _source, characterData)
		end
	end)
end)
...
AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
	...
	if not databaseConnected then
		deferrals.done(('[ESX Multicharacter] ESX Cannot Connect to your database. Please make sure it is correctly configured in your server.cfg')
		:format(oneSyncState))
	end
